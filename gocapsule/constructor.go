...
func exportConstructorFacts(pass *analysis.Pass) {
	// Function declarations only occur at the top level of a file.
	for _, file := range pass.Files {
		for _, decl := range file.Decls {
			if funcDecl, ok := decl.(*ast.FuncDecl); ok {
				exportConstructorFact(pass, funcDecl)
			}
		}
	}
}

// exportConstructorFact exports an EncapsulatedType fact for the type
// returned by funcDecl when funcDecl is a New** constructor.
func exportConstructorFact(pass *analysis.Pass, funcDecl *ast.FuncDecl) {
